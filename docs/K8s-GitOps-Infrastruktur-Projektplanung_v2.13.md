...
| 04.08.2026 | 2.14 | DEV-Optimierung ESXi-CPU-Last: zot-Registry `search`/CVE-Extension als Ursache identifiziert (~2,9 Cores Dauerlast im Leerlauf) und behoben (search+ui deaktiviert, replicaCount 3->2), esx-test von 88-99% auf ~65-80%. Kyverno/Trivy als vernachlaessigbar bestaetigt (nicht abgeschaltet). etcd-Server-Monitoring via `additionalScrapeConfigs` etabliert (kubeEtcd-Endpoints von ArgoCD ausgeschlossen), fsync/commit p99 ~3-6ms (gesund). vSphere-Dashboard nach Host-Rename (IP->DNS, 03.08.) korrigiert; label_replace-Merge verworfen (bricht Grafana-Panels). esx1 aus vCenter-Monitoring + Dashboard entfernt (alloy-Filter + Host-Variable). Details: `docs/optimierung-2026-08-04-esx-cpu-zot-etcd.md` |
