...
| 04.08.2026 | 2.14 | DEV-Optimierung ESXi-CPU-Last: zot-Registry ~2,9 Cores Dauerlast im Leerlauf. Eigentliche Ursache = `sync`-Extension (pollInterval-Mirror ghcr `dhenkeeneg/**` alle 15 Min gegen S3); entfernt, onDemand-Pull-Through bleibt -> zot ~8m. (Zuerst faelschlich `search`/CVE vermutet - Neustart-Artefakt; search+ui bleiben trotzdem aus.) replicaCount 3->2. esx-test von 88-99% auf im Mittel ~65-75% (Lastspitzen weiterhin bis ~90% wegen kleinem Host). Kyverno/Trivy als vernachlaessigbar bestaetigt (nicht abgeschaltet). etcd-Server-Monitoring via `additionalScrapeConfigs` etabliert (kubeEtcd-Endpoints von ArgoCD ausgeschlossen), fsync/commit p99 ~3-6ms (gesund). vSphere-Dashboard nach Host-Rename (IP->DNS, 03.08.) korrigiert; label_replace-Merge verworfen (bricht Grafana-Panels). esx1 aus vCenter-Monitoring + Dashboard entfernt (alloy-Filter + Host-Variable). Details: `docs/optimierung-2026-08-04-esx-cpu-zot-etcd.md` |
