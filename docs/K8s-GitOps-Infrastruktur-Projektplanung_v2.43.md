...
## Projektplanung - Version 2.43

**Erstellt:** 04.02.2026  
**Letzte Aktualisierung:** 18.08.2026 (OpenProject-Update 17.1.2 -> 17.7.2 in DEV + TEST + PROD VOLLSTAENDIG abgeschlossen. Kein Patch-Update, sondern sechs Minor-Versionen mit >100 Schema-Migrationen, davon fuenf destruktive (drop_table scheduled_meetings + version_settings, remove_column custom_fields.position_in_custom_field_section + sprints.sharing, DELETE wiki aus enabled_modules). **Strukturelles Verfahren geaendert - Seeder-Hook PostSync -> PreSync:** Der Seeder-Job (fuehrt db:migrate + db:seed aus) lief als PostSync-Hook, d.h. ArgoCD rollte erst die neuen Pods aus und migrierte danach -> neuer Code gegen altes Schema; bei nicht-ready web-Pod bleibt der Sync auf Progressing und der PostSync-Hook feuert nie (Deadlock). Umgestellt auf PreSync in allen drei Envs; entscheidende Eigenschaft: schlaegt der Hook fehl, bricht der Sync ab und die ALTEN Pods laufen unangetastet weiter. activeDeadlineSeconds 600 -> 1800 (DEV) bzw. 3600 (TEST/PROD, gebuendelter Lauf). **Neue Pflicht-Konfiguration ab 17.5.0 - SSRF-Schutz:** OpenProject blockt ab 17.5.0 ausgehende HTTP-Requests in reservierte Netze (10/8, 172.16/12, 192.168/16, 127/8, 169.254/16), auch bei Hostnamen die dorthin aufloesen; betrifft laut Doku ausdruecklich OIDC-Provider, File-Storages, Webhooks, Jira-Migrator, ab 17.6 auch Wiki-Integrationen. Ohne Gegenmassnahme waeren Keycloak-OIDC (Traefik-LB) und Garage-S3-Attachments (Service-CIDR) ausgefallen - bei gruenen Pods, gruenem Health-Check und gruenem ArgoCD. `OPENPROJECT_SSRF__PROTECTION__IP__ALLOWLIST` an web+worker aller drei Envs, einheitliche Liste: 192.168.178.0/24 (PROD) + 192.168.179.0/24 (TEST) + 192.168.180.0/24 (DEV) + 192.168.161.0/24 (Servernetz inkl. NAS/externes S3) + 10.43.0.0/16 (Service-CIDR) + 10.42.0.0/16 (Pod-CIDR). Nicht betroffen: Image-Pulls (containerd, nicht OpenProject), LDAP (kein HTTP), SMTP (kein HTTP, externer Host). **Ab 17.4.0 zwingend SECRET_KEY_BASE** (CVE-2026-46386, Container bootet sonst nicht) - war in allen drei Envs bereits gesetzt, kein Eingriff. **Hocuspocus von `latest` auf Core-Version gepinnt** (17.7.2): `latest` wird laufend neu gebaut, ein Pod-Restart zog bisher einen unbekannten Build ohne Rollback-Punkt; Beleg fuer die Beweglichkeit: unter demselben `latest` lief DEV am 17.08. auf Hocuspocus v3.4.4, TEST am 18.08. auf v4.3.0. Memcached 1.6.40 -> 1.6.45-alpine. **Vorgehen:** DEV gestaffelt in sechs Etappen (17.2.4 -> 17.3.4 -> 17.4.1 -> 17.5.1 -> 17.6.0 -> 17.7.2, je mit 3 Backups), TEST + PROD danach als Direktsprung in je zwei Commits (Vorbereitung ohne Versionswechsel als PreSync-Trockenlauf, dann Upgrade) - jede Migration war auf DEV mit echten Daten validiert. Migrationsdauer: DEV Summe ~2 Min ueber 6 Etappen, TEST 107 Migrationen in 100 s, PROD in 42 s. Downtime je Env ~3-7 Min (`replicas: 1` + `strategy: Recreate`, erzwungen durch RWO-Longhorn-Volume fuer /tmp). **Befund Repo-Struktur:** `kubernetes/base/apps/openproject/` ist toter Code - keine ArgoCD-Application referenziert `path: kubernetes/base/apps`, jede Umgebung haelt eine vollstaendige eigene deployment.yaml. Das ursprueglich geplante Environment-Pinning (Kustomize images-Patch) war deshalb unnoetig. **Befund toter Auth-Provider:** die ~20 Zeilen `OPENPROJECT_OPENID__CONNECT_KEYCLOAK_*` sind wirkungslos - der Seeder meldet in jedem Lauf `*** Skipping EnvData::OpenIDConnect::ProviderSeeder`, es existiert kein Keycloak-Button, produktive Anmeldung laeuft ueber LDAP. Aufraeumen offen. **Lessons:** (1) Hook-only-Aenderungen loesen KEINEN ArgoCD-Sync aus - Ressourcen mit `argocd.argoproj.io/hook` sind aus dem Diff ausgenommen, die App bleibt Synced; Trockenlauf braucht einen zusaetzlichen echten Manifest-Change oder manuellen Sync. (2) `kubectl get backup` kollidiert mit dem MariaDB-Operator -> immer `backups.postgresql.cnpg.io` bzw. `backups.velero.io` vollqualifiziert. (3) Nach dem Sprung `good_jobs` auf fehlgeschlagene Backfill-Jobs pruefen: `Backlogs::MigrateVersionSprintJournalsJob` scheiterte in DEV und PROD mit `NameError (uninitialized constant Backlogs)` in einem Wettlauf mit dem GoodJob-Scheduler-Start, in TEST lief er beim 3. Versuch durch - kein Log, kein Pod-Status und kein ArgoCD-Health zeigt das. (4) Einmalige `rails runner`-Laeufe NIE per `kubectl exec` in den laufenden Worker (OOMKilled Exit 137 bei 1Gi-Limit, Worker startete neu) sondern als eigener Job mit eigenem Limit. (5) Worker-Limit PROD 1Gi -> 2Gi erhoeht (DEV/TEST bewusst auf 1Gi belassen). (6) Auf k8s-mgmt-10 zeigte der kubectl-Default-Context auf k8s-test, die argocd-CLI aber auf DEV - Velero-Backup landete dadurch einmal im falschen Cluster; `--context`/`--kubecontext` immer explizit setzen. (7) `~/git/eneg-k8s-infrastructure-v2` existiert auf MacMini UND k8s-mgmt-10 -> in Anleitungen absolute Pfade + `hostname`-Vorabpruefung. Docs: guides/phase-6.3-openproject-update-guide-v2.md (NEU, ersetzt v1))  
**Vorherige Aktualisierung:** 21.07.2026 (Velero S3-Migration NAS10->NAS20 TEST+PROD VOLLSTAENDIG erledigt -> OF-2 GESCHLOSSEN, letzter aktiver NAS10-Rueckstand beseitigt. Velero war bei der P4-Migration NICHT mit umgezogen (checksumAlgorithm-Fix 14.07. hatte NAS10/HTTP bewusst beibehalten, NAS20 auf P4 verschoben - blieb dann offen). Vollstaendiger NAS10-Scan aller drei Cluster (Live + Repo-Grep) ergab: CNPG/Thanos/Loki/MariaDB/rclone (garage/odoo/idoit) in DEV/TEST/PROD bereits auf NAS20; einziger aktiver Rueckstand = Velero-BSL in TEST+PROD (`http://nas10.eneg.de:8010` + insecureSkipTLSVerify). Umbau nach DEV-Muster (14a-cleanup): BSL s3Url -> `https://nas20.eneg.de:8010`, insecureSkipTLSVerify entfernt, sauberes TLS via CA-Bundle (Secret velero-s3-ca, Sectigo R36+R46, kein SOPS - byte-identisch aus DEV kopiert) + `AWS_CA_BUNDLE=/etc/ca/ca.crt` (configuration.extraEnvVars) + extraVolumes/extraVolumeMounts Mount /etc/ca; checksumAlgorithm:"" beibehalten. Je Env 3 Dateien: velero-secrets/velero-s3-ca.yaml (neu) + velero-secrets/kustomization.yaml (resources-Block) + velero/values-override.yaml. Ablauf Variante A (Windows pusht Dateien zuerst, mgmt-10 SOPS-Reencrypt velero-s3-credentials auf NAS20-Keys als 2. Commit - truncation-sicher via temp im secrets-Verzeichnis). Reihenfolge TEST -> verifiziert -> PROD. Learnings: (1) velero-s3-ca fehlte in TEST+PROD (nur DEV hatte es) -> musste neu angelegt werden; (2) Secret wurde erst NACH velero-Refresh aktualisiert -> BSL Unavailable, Fix `kubectl rollout restart deployment/velero` (Pod cached S3-Creds beim Start, liest sie nicht zur Laufzeit neu); (3) nach BSL-Wechsel laufen alte Kopia-Maintain-Jobs kurz in "backup repository is not ready: re-establish on BSL change" - erwarteter Uebergangs-Nachlauf, kein Fehler, loest sich beim naechsten Backup je Namespace (evtl. kurzer VeleroKopiaMaintainPersistentFailures-Alert). Live verifiziert beide Envs: BSL Available gegen NAS20, CA-Mount+AWS_CA_BUNDLE aktiv, Test-Backup Completed 0 Errors/0 Warnings (TEST 252/252 Items, PROD 1126/1126). Alle Backup-/Storage-Clients in allen 3 Clustern jetzt auf NAS20 - kein aktiver NAS10-Drift mehr. Verbleibende nas10-Fundstellen im Repo nur noch: Doku-Historie, Templates/Kommentare, ueberschriebene base/-Legacy-Bloecke (CNPG nutzt ObjectStore-CRD, garage/odoo/idoit Env-Overlays gewinnen), bewusst deferrte Zot/Trivy-DEV-Punkte - alles funktional irrelevant. Docs: phases/phase-14-p1-velero-nas20-test-prod.md, migration/dev-to-test-prod-drift-2026-07.md (OF-2 geschlossen))  
**Vorherige Aktualisierung:** 17.07.2026 (Thanos-Query Sidecar-Discovery-Fix (base, alle drei Cluster) - migrationsunabhaengiger Nebenbefund aus P4a, KEIN Teil der Drift-Angleichung P1-P5 (die bleibt abgeschlossen). Problem: `thanos-query` fand den Prometheus-Sidecar-Store nicht, weil der referenzierte Service `kube-prometheus-stack-thanos-discovery` in KEINEM Cluster existierte (kube-prometheus-stack legt ihn nur bei `prometheus.thanosService.enabled: true` an - nirgends gesetzt) -> DNS-SRV-Lookup ins Leere (`no such host`, ~alle 30s, sogar doppelt), Query kannte nur die Storegateway (S3/NAS20-Historie), NICHT den Sidecar (juengste ~2h). Fix Weg B (base): in `base/monitoring/thanos/values.yaml` `query.dnsDiscovery.sidecarsService` auf den real existierenden, operator-verwalteten headless-Service `prometheus-operated` (Port `grpc`/10901) umgebogen + `query.stores: []` gesetzt (bereinigt den doppelten `--endpoint`, den stores + dnsDiscovery gemeinsam erzeugten). Weg A (`thanosService.enabled: true`) verworfen: haette einen redundanten Zweit-Service + Prometheus-Reconcile bedeutet. Handoff-Annahme "prometheus-operated evtl. ohne benannten gRPC-Port" durch Live-Pruefung widerlegt (Port ist `name: grpc`/10901). base-Change -> alle drei Cluster; Rollout ohne manuellen Eingriff (DEV/TEST Auto-Sync, PROD selfHeal Child-App), nur `thanos-query` neu gestartet, Prometheus unberuehrt. Live verifiziert DEV+TEST+PROD: Args = genau 1 Sidecar (`prometheus-operated`) + 1 Storegateway, keine SRV-Fehler mehr, Log `adding new sidecar with [storeEndpoints rulesAPI exemplarsAPI targetsAPI MetricMetadataAPI]` je Cluster, Storegateway weiter "up" (keine Regression), Pods 1/1 0 Restarts. Docs: phases/thanos-query-sidecar-discovery-fix.md, migration/dev-to-test-prod-drift-2026-07.md (Nebenbefund erledigt))  
**Vorherige Aktualisierung:** 17.07.2026 (P5 Drift-Angleich Longhorn-Settings VOLLSTAENDIG erledigt -> GESAMTE Drift-Angleichung P1-P5 ABGESCHLOSSEN, OF-6 GESCHLOSSEN. `replicaAutoBalance` (best-effort->least-effort) und `orphanResourceAutoDeletion` (->replica-data, +GracePeriod 300) von DEV-only-Overlay nach `base/longhorn/values.yaml` gezogen (Weg A/base): gelten jetzt cluster-weit fuer DEV/TEST/PROD, Drift strukturell dauerhaft aufgeloest. DEV-Overlay entschlackt (`defaultSettings: {}`, valueFiles-Struktur bewusst erhalten). Begruendung least-effort: best-effort triggert permanente Rebuilds fuer 1-Replica-Volumes (CNPG-WAL strict-local, aus P2) -> staendige System-Snapshots + unnoetige I/O-Last; least-effort ist die konservativere (I/O-aermere) Einstellung, gilt fuer TEST/PROD identisch (gleiche strict-local-Architektur). orphanResourceAutoDeletion: reine Konsistenz (kein Sicherheitsgewinn, war NICHT Ursache Incident 05.07.), Typ-Liste in v1.9 (nicht true/false), nur replica-data. Beide Aenderungen nicht-disruptiv (kein Pod-Restart/Volume-Detach). Rollout: base-Edit -> Push -> ArgoCD Hard-Refresh aller drei `longhorn`-Apps -> Sync (alle Synced/Healthy, Commit fcacc74). Live verifiziert alle drei Cluster: `settings.longhorn.io` = least-effort / replica-data, `status.applied=true` ueberall (defaultSettings-Merge griff auf Anhieb, KEIN manueller manager-Reconcile noetig). backup-alerts-overdue-Patch (36h/idoit) bewusst DEV-only belassen (OF-6-Rest, env-Anpassung noetig). Docs: migration/dev-to-test-prod-drift-2026-07.md (P5+OF-6 geschlossen, GESAMT-Drift abgeschlossen), phases/phase-13-kyverno-longhorn-stabilization-dev.md (TEST/PROD-Rollout ergaenzt))  
**Vorherige Aktualisierung:** 17.07.2026 (P4b Drift-Angleich Backup-Dienste S3-Migration NAS10->NAS20 PROD VOLLSTAENDIG erledigt -> P4 (a+b) KOMPLETT, OF-8 GESCHLOSSEN: NIEDRIG-Block (MariaDB PhysicalBackup, garage-backup, odoo-backup, idoit-backup, 17.07.) + HOCH-Block CNPG erp+shared (ObjectStore + pg_dumpall) auf NAS20, live verifiziert. Weg B (gemeinsames cnpg-s3-credentials, gemeinsamer Cutover, danach je Cluster Base-Backup+Verify), serverName Standard (kein vN), Retention 7d WAL / 32d logisch unveraendert. CA-Bundle cnpg-s3-ca (Sectigo R36+R46, kein SOPS) aus Live mariadb-s3-ca byte-identisch. Ablauf: Probe-Pod (postgres:17 + apt awscli 2.23.6, List/Put/Get/Delete NAS20 OK) -> Dateien gebaut+validiert (dry-run/kustomize/CA-byte-Vergleich) -> Push -> Remote-Check (7 Dateien + CA byte-identisch in origin/main) -> mgmt-10 cnpg-s3-credentials NAS20-Umverschluesselung -> Sync-Reihenfolge cnpg-secrets (CA+Endpoint+kombinierter Key s3-k8s-prod:..., 3 Keys keine NAS10-Reste) -> cnpg-cluster Hard-Refresh (Objectstores https/nas20+CA+Checksum) -> cnpg-logical-backup (CronJobs NAS20+ca-Volume). Verifiziert: cnpg-erp WAL-Archiving Sidecar-Log https/nas20 ~0.6s/WAL + Base-Backup completed 20260717T134344 (7D->7E) + pg_dumpall 1.1MiB; cnpg-shared analog Base-Backup 20260717T134407 (E2->E3) + pg_dumpall 188KiB. NEUE Learnings HOCH-Block: (1) ObjectStore-Cutover triggert NICHT zwingend Pod-Restart - cnpg-erp startete auto neu, cnpg-shared NICHT -> CA-Volume fehlte -> Retention-Delete "SSL validation failed [Errno 2]" (WAL-Archiving lief mit alter NAS10-Env weiter, kein Datenverlust), Fix manueller `kubectl cnpg restart cnpg-shared`; nach Cutover IMMER Pod-startTime beider Cluster pruefen; (2) Retention-Delete-Fehler != WAL-Archive-Fehler (unkritisch, verschwindet nach Restart+1.Base-Backup); (3) CA immer gegen Live verifizieren statt aus Vorlage kopieren (erster Schreibversuch hatte verfaelschtes Byte Index 1737 Root-R46, gleiche Laenge, nur per Zeichen-Diff gefunden; TEST-Datei war NICHT betroffen); (4) instanceSidecarConfiguration liegt unter .spec (nicht .spec.configuration). Alle S3-Dienste in DEV/TEST/PROD jetzt auf NAS20, kein NAS10-Drift mehr. Nur noch P5 (Longhorn replica-auto-balance) offen. Docs: phases/phase-14-p4b-nas10-nas20-prod.md, migration/dev-to-test-prod-drift-2026-07.md (OF-8 geschlossen))  
**Vorherige Aktualisierung:** 16.07.2026 (P4b Drift-Angleich Backup-Dienste S3-Migration NAS10->NAS20 TEST VOLLSTAENDIG erledigt: NIEDRIG-Risiko-Block (MariaDB PhysicalBackup, garage-backup, odoo-backup, idoit-backup) + HOCH-Risiko-Block (CNPG erp+shared ObjectStore + pg_dumpall) auf NAS20 umgestellt und live verifiziert. Weg b (Clean Cutover, Standard-serverName, frisches Base-Backup je Cluster), Weg B fuer CNPG-Secret (gemeinsames cnpg-s3-credentials, gemeinsamer Cutover, danach je Cluster Base-Backup+Verify - = DEV-Weg). Retention env-gewollt unveraendert (7d WAL, 32 Tage logisch, 168h MariaDB). CA-Bundle Sectigo R36+R46 pro Namespace (kein SOPS). Verifiziert: garage 267KiB, odoo 2,2MiB, idoit NAS20-Zugriff, MariaDB Auto-Backup; CNPG beide Cluster Archiving=True + Base-Backup completed (erp backupId 20260716T112126 tl11, shared 002) + pg_dumpall (erp 930KiB, shared 189KiB). Stolpersteine (real, alle in TEST): (1) QuObjects-Access-Key ist kombiniert <user>:<token> in ACCESS_KEY_ID, Secret-Key separat - NICHT am ":" trennen (Odoo 403 SignatureDoesNotMatch); (2) KSOPS/ArgoCD Merge-Apply -> alte Keys als Karteileichen bei Umbenennung NAS10_*->NAS_*, Live-Secret loeschen (selfHeal); (3) MariaDB PhysicalBackup.spec.storage immutable -> loeschen+neu; (4) SOPS truncation-sicher (temp im secrets-Verzeichnis, nie direkt in .enc.yaml); (5) CNPG Rolling-Restart mit Longhorn-Reattach ~4-5min/Cluster, Base-Backup erst bei 3/3, verwaiste Backup-CRs im started-Status loeschen+neu; (6) CNPG Status-Writeback unzuverlaessig -> Backup-CR autoritativ. PROD offen (Handoff: handoff/P4b-PROD-Cutover-Handoff.md). Docs: phases/phase-14-p4b-nas10-nas20-test.md, migration/dev-to-test-prod-drift-2026-07.md (P4b TEST erledigt))  
**Vorherige Aktualisierung:** 16.07.2026 (P4a Drift-Angleich Thanos S3-Migration NAS10->NAS20 VOLLSTAENDIG erledigt (TEST + PROD): beide Cluster analog auf NAS20 umgestellt - Bloecke NAS10->NAS20 kopiert (volle Historie, Compactor-Pause via compactor.enabled:false), Secret thanos-objstore-config (endpoint nas20, insecure:false, ca_file:/etc/ca/ca.crt) + CA-Mounts an compactor/storegateway (monitoring-thanos) und Prometheus-Thanos-Sidecar (monitoring: prometheusSpec.volumes+thanos.volumeMounts) split-brain-sicher in einem Cutover; eneg-s3-ca aus Loki-Migration wiederverwendet (kein neues Secret); Secret-first-Sync. Live verifiziert je Cluster: Storegateway laedt Bloecke von NAS20 (TEST 15, PROD 9; kein x509), Compactor Cleanup+Metadata-Sync sauber, Sidecar ready mit CA (Upload beim naechsten 2h-Cut), Query nutzt Storegateway. Alle Pods 1/1. Alle drei Cluster (DEV/TEST/PROD) nun Thanos+Loki auf NAS20. Stolpersteine (nur TEST, in PROD vermieden): (1) compactor.replicaCount wirkungslos beim bitnami-Chart -> enabled:false; (2) falscher secret_key -> InvalidAccessKeyId trotz TLS-OK (Access-Key-Format s3-k8s-{env}:<key>); (3) ArgoCD "Synced" ohne echten Apply -> expliziter Sync + Verifikation operationState. Nebenbefund (NICHT P4a, fuer spaeter): Query sidecarsService kube-prometheus-stack-thanos-discovery existiert in KEINEM Cluster (real: prometheus-operated:10901) -> Historie via Storegateway, eigener base-Fix. P4b (CNPG-backup, MariaDB-physical, Garage, Odoo, i-doit NAS10->NAS20) weiterhin offen. Docs: phases/phase-14-p4a-thanos-nas20-test-prod.md, migration/dev-to-test-prod-drift-2026-07.md (P4a komplett))  
**Vorherige Aktualisierung:** 16.07.2026 (P4a Thanos TEST erledigt - siehe v2.36; PROD folgte am selben Tag)  
**Vorherige Aktualisierung:** 15.07.2026 (P3 Drift-Angleich Monitoring-Resilienz VOLLSTAENDIG erledigt (TEST + PROD): Prometheus retentionSize ergaenzt (TEST 14GB/20Gi-PVC, PROD 38GB/50Gi-PVC), retention 15d env-spezifisch belassen (Thanos haelt Langzeit-Historie), Memory-Limit auf 3Gi/1Gi vereinheitlicht (alle drei Cluster gleich); TSDB-Health-Alerts (PrometheusTSDBCompactionsFailing/WALCorruptions/StorageFillingUp) aus DEV nach TEST+PROD portiert. Live verifiziert: CR-Werte greifen, Prometheus-Pod-Neustart sauber (3/3, 0 Restarts, kein OOMKill), Alert-Regelgruppen geladen (health=ok, state=inactive, kein Fehlalarm). Lesson: retentionSize schuetzt nur persistierte Bloecke NICHT das WAL (21.06.-Deadlock-Schutz = TSDB-Health-Alerts); Job-Filter kube-prometheus-stack-prometheus zwingend (Metrik existiert auch mit job=thanos-compactor). Docs: migration/dev-to-test-prod-drift-2026-07.md (P3 komplett, OF-4 entschieden))  
**Vorherige Aktualisierung:** 15.07.2026 (P2 Drift-Angleich DB-Resilienz-Haertung VOLLSTAENDIG erledigt (TEST + PROD): PriorityClass-App + WAL-Volume 5Gi->8Gi (Online-Resize via spec.walStorage.size, kein Recreate noetig - Korrektur der frueheren Annahme) + max_slot_wal_keep_size 6GB + priorityClassName eneg-stateful-critical an cnpg-erp/cnpg-shared/mariadb-galera; Galera per IST (kein SST); alle Cluster live verifiziert (WAL 8Gi, SHOW=6GB, priority 900000000, Slots aktiv, Alerts feuern nicht). Alle drei Cluster (DEV/TEST/PROD) nun identisch gehaertet. Docs: migration/dev-to-test-prod-drift-2026-07.md (P2 komplett), phases/resilienz-haertung-wal-deadlock-dev.md (TEST+PROD-Rollout abgehakt))  
...
**Velero Checksum-Fix TEST+PROD 14.07.2026 abgeschlossen (P1 Drift-Angleich):** 14.07.2026 (`checksumAlgorithm: ""` + Resource-Limits velero 2Gi/nodeAgent 1Gi nach TEST + PROD nachgezogen, NAS10/HTTP beibehalten (OF-2, NAS20 auf P4 verschoben); beide live verifiziert (BSL Available, Limits aktiv, Test-Backup Completed ohne InvalidDigest); PROD-Cleanup PartiallyFailed-Lauf 07.07.; Velero damit in allen 3 Clustern angeglichen)  
**Velero NAS10->NAS20-Migration TEST+PROD 21.07.2026 abgeschlossen (OF-2 GESCHLOSSEN):** 21.07.2026 (der bei P1 auf P4 verschobene NAS20-Umzug fuer Velero nachgeholt - Velero war der EINZIGE verbliebene aktive NAS10-Client. BSL TEST+PROD auf `https://nas20.eneg.de:8010` + CA-Bundle (velero-s3-ca, Sectigo R36+R46) statt insecureSkipTLSVerify, AWS_CA_BUNDLE + Volume-Mount /etc/ca nach DEV-14a-Muster; velero-s3-credentials via SOPS auf NAS20-Keys. Pod-Restart noetig (S3-Creds nur beim Start gelesen). Beide Envs live verifiziert: BSL Available, Test-Backup Completed 0 Fehler (TEST 252 Items, PROD 1126 Items). Alle Backup-/Storage-Clients aller 3 Cluster jetzt auf NAS20 - kein aktiver NAS10-Drift mehr. Docs: phases/phase-14-p1-velero-nas20-test-prod.md)  
**Incident DEV 21.06.2026 abgeschlossen:** 21.06.2026 (Prometheus TSDB-WAL-Deadlock: PVC 20Gi voll durch WAL-Korruption -> blockierte Kompaktierung; behoben via PVC-Expansion auf 25Gi + Memory-Limit 3Gi; 3 neue TSDB-Health-Alerts + Runbook; TEST+PROD-Rollout ausstehend)  
**NAS-Reboot-Verifikation + CNPG-Backup-Cleanup 14.07.2026 abgeschlossen:** 14.07.2026 (beide NAS nach Zertifikatstausch/Reboot ueber alle 6 CNPG-Cluster verifiziert; cnpg-erp TEST reaktiviert; suspend:false + monitoring-Dedup vereinheitlicht; PROD cnpg-erp Recovery-Konfig + serverName vN entfernt -> PROD an DEV/TEST angeglichen)  
**DB-Resilienz-Haertung TEST + PROD 15.07.2026 abgeschlossen (P2 Drift-Angleich, VOLLSTAENDIG):** 15.07.2026 (PriorityClass-App `priorityclasses` nach TEST (14.07.) + PROD (15.07.); WAL-Volume cnpg-erp+cnpg-shared 5Gi->8Gi per Online-Resize (`spec.walStorage.size`, ohne Longhorn-Deadlock, Detach-Trick nicht noetig - widerlegt die fruehere "nur per Recreate"-Annahme aus dem disk-full-Incident 29.06.; PROD-shared-Volumes fast leer -> ganz ohne Eingriff); `max_slot_wal_keep_size: 6GB` (sighup, live `SHOW`=6GB); `priorityClassName: eneg-stateful-critical` an cnpg-erp/cnpg-shared (via `kubectl cnpg restart`) und mariadb-galera (Operator-Auto-Restart, alle 3 Nodes per IST kein SST, galera-1 ohne Memory-Probleme); alle Pods priority 900000000, alle CNPG 3/3, Galera Ready/GaleraReady; Slots aktiv + CnpgClusterNoPrimary/SlotInactive feuern nicht. Lesson: CNPG-priorityClass ist kein restart-ausloesendes Feld (expliziter `cnpg restart`); Primary-Terminating kann lange dauern, loest sich selbst, kein Force-Delete. PROD-Besonderheit: `prod-infrastructure` ohne Auto-Sync (manueller Sync), `cnpg-cluster`/`mariadb-cluster` mit Auto-Sync. Alle drei Cluster (DEV/TEST/PROD) jetzt identisch gehaertet. Docs: migration/dev-to-test-prod-drift-2026-07.md, phases/resilienz-haertung-wal-deadlock-dev.md)  
**Monitoring-Resilienz TEST + PROD 15.07.2026 abgeschlossen (P3 Drift-Angleich, VOLLSTAENDIG):** 15.07.2026 (Prometheus `retentionSize` ergaenzt: TEST 14GB (~70% der 20Gi-PVC), PROD 38GB (~76% der 50Gi-PVC) - schliesst die Luecke "retention 15d ohne Groessendeckel" (retention-getriebenes PVC-Volllaufen); `retention` 15d in TEST+PROD bewusst belassen (env-spezifisch, Thanos haelt Langzeit-Historie auf S3, retentionSize ist der harte Deckel "was zuerst greift"); Memory-Limit 2Gi->3Gi / Request 512Mi->1Gi in TEST+PROD (Angleich an DEV, Puffer fuer WAL-Replay nach Pod-Neustart - PROD lag bei ~1,7GB Normallast nur ~0,3GB unter altem 2Gi-Limit) -> alle drei Cluster einheitlich 3Gi/1Gi. TSDB-Health-Alerts (`prometheus-tsdb-health-{test,prod}.yaml`) aus DEV abgeleitet: PrometheusTSDBCompactionsFailing, PrometheusTSDBWALCorruptions, PrometheusStorageFillingUp (>80% PVC, relativ), je in `monitoring-alerts/kustomization.yaml` ergaenzt. Rollout via ArgoCD-Apps `monitoring` (Helm-values, Hard-Refresh) + `monitoring-alerts` (kustomize). Live verifiziert TEST+PROD: CR-Werte greifen, Pod-Neustart sauber (3/3, 0 Restarts, kein OOMKill; PROD-Mem nach Replay ~1GB), Alert-Regelgruppen geladen (`health=ok`, alle `state=inactive` -> kein Fehlalarm). Lessons: (1) retentionSize begrenzt nur persistierte Bloecke, NICHT das WAL -> gegen den WAL-Korruptions-Deadlock (21.06.) wirken ausschliesslich die TSDB-Health-Alerts; (2) Job-Filter `kube-prometheus-stack-prometheus` ZWINGEND, da `prometheus_tsdb_compactions_total` auch mit `job=thanos-compactor` existiert. OF-4 entschieden+erledigt. Docs: migration/dev-to-test-prod-drift-2026-07.md (P3 komplett))  
**Standort:** Hamburg  
...
| Komponente | Version | Status |
|------------|---------|--------|
| CloudNativePG Operator | 1.28.3 (Chart 0.27.1, image.tag-Override) | ✅ Installiert (DEV; TEST/PROD ausstehend) |
...
| CNPG Cluster ohne Primary | - | Kein aktiver Primary >15 min (WAL-Deadlock-Vorbote) |
...
Umgebungsspezifische PrometheusRules unter `environments/<env>/monitoring-alerts/`
(ArgoCD-App `monitoring-<env>-alerts`). Erweitern/ersetzen die Default-Rules des
kube-prometheus-stack.

**Prometheus TSDB-Health (DEV, seit 21.06.2026)**

Datei: `environments/dev/monitoring-alerts/prometheus-tsdb-health-dev.yaml`
(PrometheusRule `prometheus-tsdb-health-dev`). Eingefuehrt nach dem TSDB-WAL-Deadlock
vom 21.06.2026 (PVC durch ungebremst wachsendes WAL vollgelaufen, weil eine
WAL-Korruption die Kompaktierung blockierte; `retentionSize` schuetzt NICHT das WAL).

| Alert | Bedingung (Kern) | for | Severity | Zweck |
|-------|------------------|-----|----------|-------|
| PrometheusTSDBCompactionsFailing | `increase(prometheus_tsdb_compactions_failed_total[1h]) > 0` | 30m | warning | Vorbote: Kompaktierung scheitert -> WAL waechst |
| PrometheusTSDBWALCorruptions | `increase(prometheus_tsdb_wal_corruptions_total[1h]) > 0` | 5m | warning | Wurzel: WAL-Korruption (meist Storage-I/O-Latenz) |
| PrometheusStorageFillingUp | `kubelet_volume_stats_used/capacity{pvc=~"prometheus-.*"} > 0.80` | 15m | warning | Letzte Sicherung: PVC >80% vor Schreibstopp |

Zugehoerige Konfig (DEV `monitoring/values-override.yaml`): Prometheus-PVC 25Gi
(vorher 20Gi), `resources.limits.memory: 3Gi` (vorher 2Gi), `retention: 7d`,
`retentionSize: 12GB`. Runbook: `docs/runbooks/prometheus-wal-deadlock.md`.

**Rollout-Status:** DEV, TEST und PROD erledigt + verifiziert (P3 Drift-Angleich
15.07.2026). TEST/PROD-Werte env-spezifisch: `retentionSize` TEST 14GB (20Gi-PVC) /
PROD 38GB (50Gi-PVC), `retention` 15d belassen, `resources.limits.memory: 3Gi` /
`requests.memory: 1Gi` einheitlich in allen drei Clustern. Alerts als
`prometheus-tsdb-health-{test,prod}.yaml` portiert (live `health=ok`, `inactive`).
...
| 29.06.2026 | 2.28    | **Resilienz-Haertung gegen WAL-Deadlock / Infra-Pod-Restart-Kaskade (DEV).** Reaktion auf das Ausfallmuster vom 29.06.2026 (separates Incident-Doc `docs/incidents/2026-06-29-cnpg-wal-deadlock-longhorn-kaskade.md`): Ein gleichzeitiger Restart mehrerer Infra-Pods (csi-resizer 66 Restarts, cnpg-operator 32, longhorn-manager) loeste eine vierstufige Kaskade aus — (1) CNPG WAL-Deadlock (beide PG-Cluster ohne Primary → kein WAL-Recycling → 8Gi-WAL-Volumes voll → `ensure_sufficient_disk_space`-Guard verhindert Pod-Start → Henne-Ei), (2) Longhorn-Expansion-Haenger, (3) iSCSI-Altlasten auf k8s-dev-21, (4) Folgeschaeden (Loki-FS-Korruption, Prometheus-Snapshot-Wachstum). Die Haertung setzt an der Wurzel (Stufe 1) an. **Vier umgesetzte DEV-Massnahmen:** (1) **`max_slot_wal_keep_size: "6GB"`** in beiden CNPG-Clustern (`spec.postgresql.parameters`, sighup-Reload ohne Restart) — begrenzt das WAL-Rueckhalten eines (auch inaktiven) Slots; bei Ueberschreitung faellt die Replica auf S3-Archive-Restore zurueck statt das WAL-Volume volllaufen zu lassen; 8Gi-Volume → 6GB-Deckel mit ~2Gi Puffer. (2) **PriorityClass `eneg-stateful-critical`** (`value: 900000000`, knapp unter `longhorn-critical` 1e9, `globalDefault: false`) NEU als cluster-globale Ressource `kubernetes/base/priorityclasses/eneg-stateful-critical.yaml` + ArgoCD-App `kubernetes/environments/dev/infrastructure/priorityclasses-app.yaml` (Sync-Wave 1, `directory.include`); eingebunden via `spec.priorityClassName` in cnpg-erp, cnpg-shared (CNPG) und mariadb-galera (MariaDB) — schuetzt die DB-Pods im Node-Pressure-Fall vor Verdraengung. Bewusst NICHT fuer Monitoring (Beobachter, speicherhungrig, soll eher weichen) und Registry/Zot (3 Replicas HA, DEV-Internet-Fallback; PROD separat in Phase 9a Etappe B via Image-Warmup, nicht Pod-Priority). (3) **Alert `CnpgClusterNoPrimary`** in `kubernetes/base/monitoring/alert-rules/cnpg-alerts.yaml` (nun 8 Rules): `count by (cluster, namespace) (cnpg_pg_replication_in_recovery{namespace="databases"} == 0) < 1`, `for: 15m`, severity critical — Frueherkennung der Henne-Ei-Lage, BEVOR sich das WAL-Volume fuellt (der Deadlock baut sich erst ueber Stunden auf, im Incident ~3,5 Tage). Ergaenzt `CnpgClusterNotReady` (Exporter-Ausfall via `collector_up==0`) ohne Ueberschneidung: Exporter weg → NotReady; Exporter da, aber kein Primary → NoPrimary. `for: 15m` (statt urspruenglich erwogener 5m, auf Wunsch erhoeht) ueberlebt jeden normalen Switchover und laengere Sonderfaelle (pg_basebackup-Join, kurze API-Server-Haenger). Liegt in `base/` → kommt beim TEST/PROD-Sync automatisch mit. (4) **CNPG-Operator-Patch 1.28.1 → 1.28.3** (Bonus, im Zuge der Analyse notwendig geworden): behebt CVE-2026-44477 (CVSS 9.4, Metrics-Exporter) + Daten-Sicherheits-Bug im Failover-Pfad. Methode: `image.tag: "1.28.3"` in `kubernetes/base/cloudnative-pg/operator/values.yaml` bei unveraendertem Chart 0.27.1 (Image-Override) — der Chart-appVersion-Pfad bietet von 1.28.1 nur den Sprung auf 1.29.1 (Chart 0.28.x = Minor-Upgrade); 1.28.3 gleiche Minor-Linie laut CNPG-Versionierungspolitik ohne Breaking-Changes. Da der PriorityClass-Rollout ohnehin Restarts ausloeste, wurde zuerst gepatcht, dann neu gestartet (auf gefixter Version). Nebenwirkung: ab 1.28.3 nutzt der Exporter die Rolle `cnpg_metrics_exporter` (pg_monitor) statt postgres-Superuser — Standard-Queries (`cnpg-default-monitoring`) nicht betroffen, im DEV-Log bestaetigt (`CREATE ROLE cnpg_metrics_exporter` + `GRANT pg_monitor`), keine Nacharbeit. **Verifikation (DEV):** CNPG-Rolling-Restart sauber (In-Place-Primary-Restart bei `primaryUpdateMethod: restart`, beide Cluster 3/3 healthy, Archiving+Backup OK); Galera-Rolling-Restart alle 3 Nodes per IST (kein SST), `ReplicasFirstPrimaryLast`, durchgehende Schreibverfuegbarkeit via autoFailover, Cluster Ready/Running, Primary wieder galera-0; alle 9 DB-Pods (6 CNPG + 3 Galera) tragen `priorityClassName: eneg-stateful-critical` (priority 900000000); `CnpgClusterNoPrimary` im Cluster, im Normalzustand inaktiv (pro Cluster genau 1 Primary → count==1); Operator-Deployment auf Image 1.28.3, 1/1. **Geaenderte Files (8):** `kubernetes/base/priorityclasses/eneg-stateful-critical.yaml` (NEU), `kubernetes/environments/dev/infrastructure/priorityclasses-app.yaml` (NEU), `kubernetes/environments/dev/cnpg-cluster/cnpg-{erp,shared}.yaml`, `kubernetes/base/cloudnative-pg/operator/values.yaml`, `kubernetes/environments/dev/infrastructure/cnpg-operator-app.yaml` (Kommentar), `kubernetes/base/monitoring/alert-rules/cnpg-alerts.yaml`, `kubernetes/environments/dev/mariadb-cluster/mariadb-galera.yaml`. **Doku:** `docs/phases/resilienz-haertung-wal-deadlock-dev.md` (NEU — Massnahmen, Verifikation, TEST/PROD-Rollout-Vorlage, offene Punkte); Incident-Doc Follow-ups aktualisiert. **Offene Folgepunkte:** (a) **TEST/PROD-Rollout** aller vier Massnahmen sequenziell DEV→TEST→PROD (Reihenfolge je Env: PriorityClass-Definition+App zuerst, dann Operator-Patch, dann CNPG-Manifeste mit env-spezifischer WAL-Volume-Groessenpruefung fuer `max_slot_wal_keep_size`, dann Galera; Alert kommt via base/ automatisch). (b) **CNPG Minor-Upgrade 1.28.x → 1.29.x** als eigenes geplantes Vorhaben (Chart 0.28.x, Review 1.29-Breaking-Changes). (c) Aus der Analyse nicht umgesetzt: Longhorn-Node-Speicher-Alert (<15% frei), Speicher-Headroom k8s-dev-22/-23 (ggf. Over-Provisioning 200%→150%), Resource-Review der neustartenden Infra-Pods (csi-resizer/cnpg-operator). (d) **Loki-FS-Korruption** (2. Vorfall Mai+Juni) weiterhin offene Root-Cause-Untersuchung. |
| 03.07.2026 | 2.29    | **vSphere/vCenter-Monitoring via OpenTelemetry vcenter-Receiver (DEV) ABGESCHLOSSEN.** Ziel: ESXi-/vCenter-Metriken (Host-CPU/RAM, Datastore-Kapazitaet+Latenz, VM-Ready-Time/Ballooning/Swap) nach Prometheus/Thanos, um zwei Fragen datengestuetzt zu beantworten: (1) Disk-/Datastore-Latenz sichtbar (strukturelles ~200ms-Problem), (2) Host-Headroom fuer Platzierung weiterer K8s-Nodes. **Architektur:** Dedizierte Alloy-Instanz (Release `alloy-vcenter`, Deployment replicas 1), bewusst GETRENNT vom Log-Forwarding-DaemonSet (Release `alloy`), da ein DaemonSet jeden vCenter von jedem Node aus pollen wuerde (Mehrfachlast + Duplikate). Beide vCenter (vcenter-a.eneg.de 8.0.3, vcenter-b.eneg.de 8.0.2) in EINER Instanz via 2x `otelcol.receiver.vcenter`, gemeinsamer batch+export-Pfad. **Datenfluss Muster P (remote_write/Push):** receiver -> `otelcol.processor.transform` (hebt vCenter-resource_attributes Host/Datastore/Cluster/VM auf Datapoint-Ebene, damit sie Prometheus-Labels bleiben; plus statisches Label vcenter=a/b) -> batch -> `otelcol.exporter.prometheus` -> `prometheus.remote_write` an kube-prometheus-stack (`enableRemoteWriteReceiver: true` in `environments/dev/monitoring/values-override.yaml`, loeste 1x Prometheus-Neustart aus). ServiceMonitor-Weg verworfen (otelcol.exporter.prometheus exponiert keinen HTTP-Endpoint). **TLS:** Beide vCenter nutzen self-signed VMCA-Zerts (vsphere.local) und liefern die Root nicht mit -> clientseitiges CA-Bundle (beide VMCA-Roots, base64) als Secret `vcenter-ca-bundle` (KEIN SOPS, da oeffentliche CA-Zerts), gemountet nach /etc/vcenter-ca/ca.crt -> ca_file; Ansprache zwingend via FQDN (SAN). **User:** dedizierter AD-User `svc-otel-vcenter@eneg.de`, Rolle `OTel-ReadOnly-Monitoring` (Read-only geklont + Performance.ModifyIntervals) am Root beider vCenter mit Propagate; Credentials als SOPS-Secret `vcenter-credentials`. **Versionen:** Alloy Helm-Chart 1.10.0 (grafana.github.io/helm-charts) -> Binary v1.17.0; vcenter-Receiver EXPERIMENTAL -> `alloy.stabilityLevel: experimental` Pflicht. **Rollout-Learning:** Chart-Version IMMER gegen index.yaml des Helm-Repos pruefen — 1.10.1 existiert nur als GitHub-Release, nicht im Helm-Index (ArgoCD-Fehler "chart not found"); korrigiert auf 1.10.0 (scripts/list-alloy-versions.ps1). Zwei-Etappen-Push (secret-first): erst Bausteine, dann SOPS-Verschluesselung auf mgmt-10, dann ArgoCD-Apps (sync-wave 1 Secret, wave 4 Instanz). **Ergebnis:** ~40 vcenter_*-Metriken in Prometheus, beide vCenter je 3 Hosts, korrekte Labels (kein Label-Bug), inkl. Disk-Latenz (`vcenter_host_disk_latency_avg/max_milliseconds`, kam nach erstem Performance-Counter-Zyklus ~10min) und Host-Memory-Capacity (per Default deaktiviert -> explizit aktiviert). Kapazitaets-Erkenntnis DEV: 192.168.160.10 (vcenter-b) mit ~252GB freiem RAM groesster Headroom. **Grafana-Dashboard** "vSphere / vCenter - Hosts & Kapazitaet" (uid `vsphere-hosts-kapazitaet`, 21 Panels, Variablen vcenter+host, Root-Ordner): Cluster-Kapazitaet, Host-CPU, Host-RAM (inkl. "frei fuer neue Nodes"), Overcommit (Ready-Time/Ballooning/Swap), Datastore-Belegung (Zeitreihe+Tabelle), Disk-Latenz (avg/max je Host + Detail je LUN). Bewusst im DEV-Overlay (`environments/dev/monitoring-alerts/`), NICHT in base -> Environment-Trennung, da Metriken vorerst nur in DEV fliessen. **Config-Nachzug:** transform-Statements auf `datapoint.attributes[...]` umgestellt (behebt Style-Warn "paths modified to include context prefix"). **Runbook:** `docs/runbooks/vsphere-vcenter-exporter-alloy.md` (NEU: Architektur, User-/CA-Setup, SOPS/Rollout Zwei-Etappen, Verifikation, Metrik-Referenz, Troubleshooting). **Geaenderte/neue Files:** `kubernetes/base/monitoring/alloy-vcenter/values.yaml` (NEU), `kubernetes/environments/dev/alloy-vcenter/values.yaml` (NEU), `kubernetes/environments/dev/alloy-vcenter-secrets/` (NEU: ca-bundle, credentials-template+enc, secret-generator, kustomization), `kubernetes/environments/dev/infrastructure/alloy-vcenter-{secrets-,}app.yaml` (NEU), `kubernetes/environments/dev/monitoring/values-override.yaml` (enableRemoteWriteReceiver), `kubernetes/environments/dev/monitoring-alerts/vsphere-hosts-dashboard-cm.yaml` (NEU) + kustomization, diverse `scripts/`. **Offene Folgepunkte:** (a) **Alerts** (naechster Schritt): Datastore-Belegung >90%, Host-Disk-Latenz-Schwelle, fast voller Datastore S2850-SSD-R5 (vcenter-b). (b) **TEST+PROD-Rollout** nach etablierter Struktur (Runbook enthaelt Anleitung; User-Berechtigung bereits vorhanden da gemeinsame AD-Source, CA-Bundle identisch, env-spezifische remote_write-external_labels, Dashboard ins jeweilige Overlay). |
| 06.07.2026 | 2.30    | **kube-vip API-Server-VIP (Control-Plane HA) DEV ABGESCHLOSSEN.** Loest den SPOF des an Node-1-IP gebundenen kubeconfig-Endpoints (dev/test/prod hingen an `.21`). Voraussetzung fuer die anstehende TEST-Node-Migration auf neuen VMware-Host (beim Verschieben von Node 1 waere sonst kubectl-/MCP-Sicht verloren gegangen). **kube-vip v1.2.0**, ARP-Modus (alle Nodes je Cluster im selben L2-Subnetz), nur Control-Plane (`cp_enable`), KEIN Service-LB (bleibt bei MetalLB). VIP DEV `192.168.180.20` (DNS `k8s-dev-api.eneg.de`), bewusst ausserhalb MetalLB-Pool (`.100` + `.151-.199`) — darf dort nie aufgenommen werden. **Deployment:** K3s-Auto-Deploy-Manifest (`/var/lib/rancher/k3s/server/manifests/`), verteilt per Ansible-Playbook `10-kube-vip-deploy.yml` (Play 1: tls-san-Erweiterung config.yaml-sync + rolling K3s-Restart `serial:1` mit hostname-Check + Ready-Check; Play 2: kube-vip verteilen; Play 3: Verify Pods/Lease/VIP). Tags `san` + `kube-vip`. `k3s_tls_san` in `inventory/dev/group_vars/all.yml` additiv um VIP + DNS erweitert (Zertifikat via openssl bestaetigt: SANs `192.168.180.20` + `k8s-dev-api.eneg.de` vorhanden, bestehende erhalten). kubeconfig-Endpoint (mgmt-10) auf VIP-DNS umgestellt (Backup + Test-Kopie-Verfahren). **Failover-Test bestanden:** Leader-Node (dev-21) gestoppt → VIP wandert in Sekunden auf dev-22 (`leaseTransitions` 3→4), Zugriff ueber VIP durchgehend, etcd-Quorum intakt, Node nach `start` sauber zurueck. **Zwei geloeste Stolpersteine (beide im Runbook dokumentiert):** (1) **Kein Failover** — kube-vip griff im `--inCluster`-Default ueber ClusterIP `10.43.0.1` auf die Lease zu (`connection refused` bei Node-Ausfall, `leaseTransitions:0`); Fix: lokale K3s-Kubeconfig `/etc/rancher/k3s/k3s.yaml` read-only nach `/etc/kubernetes/admin.conf` mounten. (2) **VIP wird nicht aufgelegt** — kube-vip spricht API ueber Namen `kubernetes`, DNS scheitert (`server misbehaving`), kein Leader; Fix: `hostAliases` mappt `kubernetes` → `127.0.0.1` (lokaler API via hostNetwork). Beide Fixes gehoeren zusammen. **Lease-Timings 5/3/1** (statt Default 15/10/2) fuer schnelles Failover. **Nebenwirkung Failover-Test:** `mariadb-galera-1` (lief auf dev-21) haengte danach auf `0/2` (`Donor/Desynced`-Readiness-Zustand haengengeblieben, DB aber synced, Operator `GaleraReady=True`, Quorum 2/3 gesund); sauber behoben via Pod-Delete (StatefulSet-Recreate + Rejoin) — Merke fuer TEST-Migration: nach Node-Stopp Galera-Pods des Nodes pruefen/neustarten. **Neue/geaenderte Files:** `ansible/files/kube-vip/kube-vip-dev.yaml` (NEU: SA+ClusterRole+Binding+DaemonSet, gehaertet), `ansible/playbooks/10-kube-vip-deploy.yml` (NEU), `ansible/inventory/dev/group_vars/all.yml` (tls-san + kube_vip_*-Vars), `docs/runbooks/kube-vip-api-vip.md` (NEU: Architektur, Design-Entscheidungen, Rollout-Prozedur DEV/TEST/PROD, Stolpersteine, Rollback, Diagnose-Schnellreferenz). **Offene Folgepunkte:** (a) **TEST-Rollout** (Voraussetzung fuer Node-Migration; VIP `192.168.179.20` / `k8s-test-api.eneg.de`, MetalLB-Pool-Check, kube-vip-test.yaml aus DEV ableiten), dann **PROD** (`192.168.178.20` / `k8s-prod-api.eneg.de`). (b) MCP-kubeconfig-Endpoint auf VIP umstellen (haengt aktuell noch an Node-IP). (c) Laptop-kubeconfig (`C:\Users\dhenke\.kube\config`) DEV-Zeile auf VIP-DNS umstellen. |
| 06.07.2026 | 2.30    | **kube-vip TEST-Rollout ABGESCHLOSSEN + inotify-Fix TEST (Nachtrag zu 2.30).** kube-vip nach identischem Muster wie DEV auf TEST ausgerollt — diesmal glatt beim Erstlauf, da beide DEV-Stolpersteine (lokale kubeconfig-Mount + hostAlias `kubernetes`) bereits im Manifest enthalten sind. VIP `192.168.179.20` / `k8s-test-api.eneg.de`, MetalLB-Pool-Check bestanden (`.100` + `.151-.199`, VIP ausserhalb). tls-san rolling erweitert (Zertifikat bestaetigt), kube-vip verteilt (Leader test-21, ARP-Broadcast sauber, keine Fehlersignaturen), kubeconfig mgmt-10 auf VIP-DNS umgestellt. **Failover-Test bestanden:** Leader test-23 gestoppt → VIP wandert auf test-22 (`leaseTransitions` 1→2), Poll-Schleife zeigt ~6s Ausfall-Luecke mit anschliessender Selbst-Erholung, etcd-Quorum intakt, Galera 3/3 gesund. **Wichtiger Nebenbefund inotify:** Beim ersten Failover-Versuch warf `systemctl stop k3s` auf k8s-test-21 `Failed to allocate directory watch: Too many open files`. Ursache: inotify-Limits auf Ubuntu-24.04-Defaults (`max_user_instances=128`, `max_user_watches` ~124126) statt eNeG-Zielwerten — das Playbook `06-sysctl-inotify-limits.yml` (Phase 9a) war auf TEST nie nachgelaufen (Nodes aelter als Fix, TEST zwischenzeitlich deaktiviert). Behoben via `ansible-playbook -i inventory/test/hosts.ini playbooks/06-sysctl-inotify-limits.yml` (alle 3 TEST-Nodes jetzt 8192/524288). **inotify-Statuspruefung alle Envs:** DEV OK (Phase 9a), TEST heute gefixt, PROD offen (Nodes aus; Packer-Template `user-data.pkrtpl.hcl` enthaelt den Fix + Playbook 06 vorhanden → Live-Pruefung bei Reaktivierung noetig). **Geaenderte/neue Files:** `ansible/files/kube-vip/kube-vip-test.yaml` (NEU), `ansible/inventory/test/group_vars/all.yml` (tls-san + kube_vip_*-Vars), `ansible/playbooks/10-kube-vip-deploy.yml` (VIP-Test-Task-Name dynamisch via `{{ kube_vip_address }}` statt hartkodiert `.180.20`), `docs/runbooks/kube-vip-api-vip.md` (Rollout-Status TEST=OK, inotify-Abschnitt ergaenzt). **TEST ist damit bereit fuer die Node-Migration auf neuen VMware-Host.** **Offene Folgepunkte:** (a) **PROD-Rollout** kube-vip (`192.168.178.20` / `k8s-prod-api.eneg.de`) sobald PROD an — inkl. vorheriger inotify-Live-Pruefung + ggf. Playbook 06. (b) MCP-kubeconfig-Endpoints (DEV+TEST) auf VIP umstellen (haengen noch an Node-IPs). (c) Laptop-kubeconfig DEV+TEST-Zeilen auf VIP-DNS umstellen. (d) Danach: TEST-Node-Migration auf neuen VMware-Host. |
| 06.07.2026 | 2.31    | **Monitoring-Reaktivierung TEST + Loki 14b (NAS20) + Loki Retention-Enforcement (base).** Nach TEST-Node-Migration + kube-vip wurde der Monitoring-Stack in TEST wieder auf DEV-Stand gebracht. **Befund A - Kern-Stack degradiert:** Prometheus + Alertmanager standen auf `replicas: 0`, `monitoring`-App hing seit 06.05. in einem PreSync-Hook. Wurzelursache: `argocd-application-controller` (StatefulSet) stand auf `0/0` — fuer die Node-Migration bewusst pausiert (Reconciliation cluster-weit angehalten). Ohne Controller kein Refresh/selfHeal/Sync-Abschluss. **Loesung:** (1) `kubectl -n argocd scale statefulset argocd-application-controller --replicas=1` — heilte nebenbei diverse Migrations-Reste (thanos ComparisonError, mariadb-cluster, cnpg-logical-backup, openproject → Healthy). (2) Prometheus/Alertmanager kamen NICHT von allein auf 1: kube-prometheus-stack rendert `replicas` nur, wenn in Values gesetzt — sonst kein ArgoCD-Diff auf das manuell gesetzte `spec.replicas: 0` (unter SSA tueckisch). **Git-Haertung** in `environments/test/monitoring/values-override.yaml`: `prometheus.prometheusSpec.replicas: 1` + `alertmanager.alertmanagerSpec.replicas: 1`. Ergebnis prometheus-0 3/3, alertmanager-0 2/2. **Lesson:** Wer Prometheus/Alertmanager fuer eine Migration auf 0 faehrt, muss den Sollwert (1) explizit in den Values haben oder das Herunterfahren ueber Git machen — sonst heilt ArgoCD die 0 nicht. **Befund B - Loki-CrashLoop (14b TEST):** loki-0 CrashLoopBackOff mit `open /etc/ca/ca.crt: no such file` → `failed to build s3 config`. Die bisherige Annahme "ca_file (base) bleibt inert bei insecure:true" ist FALSCH — Loki 3.6.7 laedt die ca_file bereits beim Config-Load (Init ruler-storage), unabhaengig von insecure. Da das CA-Volume in TEST nie gemountet war, Crash. **Loesung = Cutover auf NAS20/HTTPS (Phase 14b, Dienst 1):** CA-Bundle-Secret `eneg-s3-ca` nach TEST (Klartext, kein SOPS, byte-identisch zu DEV) + kustomization `resources`-Eintrag; NAS20-S3-Credentials in `loki-s3-credentials.enc.yaml` aktualisiert (SOPS, User `k8s-test-s3`); Values-Override: NAS10/HTTP-Block entfernt (endpoint+insecure aus base), Bucket `k8s-test-loki` bleibt (jetzt NAS20), CA-Volume-Mount nach /etc/ca ergaenzt (analog DEV), `retention_period: 120h` (5d). Stolperstein: erster NAS20-Key mit `InvalidAccessKeyId (403)` abgelehnt → Key korrigiert (2. sops-Durchlauf) → Index-Sync `success=true`. Cutover secret-first (loki-secrets syncen + `kubectl get secret` verifizieren, DANN Values pushen; Loki-Credentials via extraEnvFrom sind NICHT live → Pod-delete noetig). loki-0 2/2 Running. **Massnahme C - Loki Retention-Enforcement (base, alle Envs):** Befund: weder base noch Runtime-Config hatten einen `compactor`-Block mit `retention_enabled` — `retention_period` war gesetzt, aber WIRKUNGSLOS (Compactor macht ohne retention_enabled nur Index-Compaction, loescht NICHTS). Betraf DEV genauso. Recherche (Loki 3.6.7): `retention_enabled: true` noetig; `delete_request_store` ab Loki 3.0 PFLICHT sonst CONFIG ERROR + CrashLoop (grafana/loki #12588); Index-Period 24h erfuellt. **base/monitoring/loki/values.yaml** unter `loki:` ergaenzt: `compactor: {retention_enabled: true, delete_request_store: s3, working_directory: /var/loki/retention, retention_delete_delay: 2h}`. Rollout Weg 1 (base-Push + gestaffelter Sync): DEV+TEST loki zogen die Aenderung, beide loki-0 sauber neu gestartet OHNE CrashLoop, compactor-Block in gerenderter ConfigMap, Log "chosen to run the compactor, starting compactor". Wirkung je Env ueber retention_period: DEV/TEST 120h (5d), PROD erbt aktuell base 2160h (90d). **Geaenderte/neue Files:** `environments/test/monitoring/values-override.yaml` (replicas-Haertung), `environments/test/monitoring-loki-secrets/eneg-s3-ca.yaml` (NEU) + kustomization + `loki-s3-credentials.enc.yaml`, `environments/test/monitoring-loki/values-override.yaml` (NAS20-Cutover + retention 120h), `base/monitoring/loki/values.yaml` (compactor). **Docs:** `docs/phases/monitoring-reaktivierung-test-2026-07-06.md` (NEU), `docs/phases/phase-14-s3-migration-nas10-nas20.md` (14b-TEST-Loki-Abschnitt ergaenzt). **Offene Folgepunkte:** (a) **PROD Monitoring-Reaktivierung** bei PROD-Start (ggf. gleiche replicas-Haertung im PROD-Override, falls dort auch auf 0). (b) **PROD Loki-Retention-Wert WIDERSPRUCH:** Phase-14 sieht "PROD 10 Tage" vor, aber PROD hat kein retention_period-Override → wuerde base 2160h (90d) erben. Vor/bei 14c PROD: `retention_period: 240h` (10d) im PROD-Loki-Override setzen. (c) **Erster Retention-Lauf beobachten (DEV/TEST):** DEV hatte bislang unbegrenzt Logs → erster Compaction-Lauf loescht alles >120h, ggf. sichtbarer NAS20-Bucket-Rueckgang; TEST erst in 5 Tagen sichtbar. (d) **14b uebrige TEST-Dienste** (Velero, MariaDB, CNPG, Thanos, Registry, Backups) — Loki war Dienst 1 von 14b. |
| 17.07.2026 | 2.40    | **P5 Drift-Angleich Longhorn-Settings (Weg A/base) ABGESCHLOSSEN -> GESAMTE Drift-Angleichung P1-P5 KOMPLETT, OF-6 GESCHLOSSEN.** Letzter offener Drift-Punkt aus `docs/migration/dev-to-test-prod-drift-2026-07.md` (Abschnitt 5.7 / Prioritaet 5) - bewusst als Grenzfall/Diskussion angelegt (Phase-13 "DO-NOT-PORT pauschal ohne Review"). **Entscheidung (Daniel):** beide Longhorn-Settings nachziehen, Weg A (base), backup-alerts-overdue-Patch bleibt DEV-only. **Ist-Zustand vor Aenderung (live verifiziert alle drei Cluster):** `replica-auto-balance` = DEV least-effort / TEST best-effort / PROD best-effort; `orphan-resource-auto-deletion` = DEV replica-data / TEST leer / PROD leer. Die DEV-Werte lagen im DEV-Overlay `environments/dev/longhorn/values-override.yaml`, base hatte `replicaAutoBalance: best-effort` + `orphanAutoDeletion: true`. **Umsetzung:** In `kubernetes/base/longhorn/values.yaml`: `replicaAutoBalance` best-effort -> least-effort; `orphanAutoDeletion: true` (alter boolean-Stil) ersetzt durch v1.9-Typ-Liste `orphanResourceAutoDeletion: "replica-data"` + `orphanResourceAutoDeletionGracePeriod: "300"` (jeweils mit ausfuehrlichem Begruendungs-Kommentar). DEV-Overlay entschlackt: die drei jetzt redundanten Override-Zeilen entfernt, `defaultSettings: {}` (leerer Block bewusst behalten, damit valueFiles-Referenz + Einhaengepunkt fuer kuenftige DEV-only-Overrides stabil bleiben). **Begruendung `least-effort`:** best-effort triggert permanente Rebuild-Operationen fuer 1-Replica-Volumes (CNPG-WAL etc. auf strict-local, aus P2) -> staendige System-Snapshots ("becomes not ready to use") + unnoetige I/O-Last; least-effort balanciert nur bei echt degradierten Volumes = konservativere (I/O-aermere) Einstellung. Gilt fuer TEST/PROD identisch, da dort dieselbe strict-local/1-Replica-Architektur existiert (P2). **Begruendung `orphanResourceAutoDeletion`:** reine Konsistenz + explizites Verhalten (war in DEV faktisch ~136d Cluster-Default), KEIN Sicherheitsgewinn - der 05.07.-Incident hat gezeigt, dass verwaiste Replicas auf gestressten/evicteten Nodes ohnehin nicht auto-geloescht werden; in v1.9 Typ-Liste (nicht true/false), nur replica-data gesetzt. **Vorteil Weg A:** Drift verschwindet strukturell dauerhaft, alle drei Cluster automatisch gleich, kein dreifach redundanter Wert, least-effort ist der objektiv bessere Default fuer unsere Architektur. Bewusst in Kauf genommen: base wirkt sofort auf alle drei Cluster gleichzeitig (kein gestaffeltes DEV->TEST->PROD) - vertretbar, da beide Aenderungen nicht-disruptiv sind (kein Pod-Restart, kein Volume-Detach) und least-effort die I/O-aermere Richtung ist. **Rollout:** base-Edit + DEV-Overlay-Entschlackung (Desktop-Commander) -> git commit/push (Daniel) -> ArgoCD Hard-Refresh-Annotation (`argocd.argoproj.io/refresh: hard`) auf alle drei `longhorn`-Application-CRs via Kubernetes-MCP -> Sync. **Verifikation (live, alle drei Cluster):** `longhorn`-App je Synced + Healthy auf Commit `fcacc74`; `settings.longhorn.io` `replica-auto-balance` = least-effort (`applied=true`) und `orphan-resource-auto-deletion` = replica-data (`applied=true`) in DEV/TEST/PROD. **Wichtige Lesson:** Der befuerchtete "klebrige" Setting-Reconcile (Longhorn uebernimmt defaultSettings nicht immer sofort in settings.longhorn.io) trat NICHT ein - der defaultSettings-Merge griff nach dem Hard-Refresh auf Anhieb in allen drei Clustern, KEIN manueller longhorn-manager-Reconcile noetig. **OF-6-Rest:** backup-alerts-overdue-Patch (36h/idoit-Selector, DEV-only) bewusst NICHT mitgenommen (DEV-spezifischer idoit-Selector + DEV-Zeitfenster, braucht env-Anpassung) - als eigenes kleines Thema fuer spaeter vermerkt. **Geaenderte Files (2):** `kubernetes/base/longhorn/values.yaml`, `kubernetes/environments/dev/longhorn/values-override.yaml`. **Docs:** `docs/migration/dev-to-test-prod-drift-2026-07.md` (P5 + OF-6 als erledigt, GESAMT-Drift-Angleichung abgeschlossen), `docs/phases/phase-13-kyverno-longhorn-stabilization-dev.md` (TEST/PROD-Rollout der Longhorn-Settings ergaenzt). **Damit ist die gesamte Drift-Angleichung DEV->TEST/PROD (P1 Velero, P2 DB-Resilienz, P3 Monitoring-Resilienz, P4 S3-NAS20, P5 Longhorn) abgeschlossen.** Einziger im Drift-Kontext dokumentierter Rest: der Query-sidecars-Service-Discovery-Fix (base, migrations-unabhaengig, eigener Chat). |
| 18.08.2026 | 2.43    | **OpenProject 17.1.2 -> 17.7.2 in DEV + TEST + PROD ABGESCHLOSSEN.** Sechs Minor-Versionen, >100 Migrationen, fuenf davon destruktiv. **Verfahrensaenderung:** Seeder-Hook PostSync -> PreSync in allen drei Envs (Migration VOR dem Pod-Rollout; schlaegt der Hook fehl, bricht der Sync ab und die alten Pods laufen weiter), activeDeadlineSeconds 600 -> 1800/3600. **Neue Pflicht-Konfiguration:** `OPENPROJECT_SSRF__PROTECTION__IP__ALLOWLIST` an web+worker - ab 17.5.0 blockt OpenProject ausgehende HTTP-Requests in reservierte Netze (auch bei Hostnamen die dorthin aufloesen); ohne Allowlist waeren Keycloak-OIDC und Garage-S3-Attachments ausgefallen, bei gruenen Pods und gruenem ArgoCD. Einheitliche Liste ueber alle Envs (178/179/180/161er-Netze + Service- und Pod-CIDR). Ab 17.4.0 ist SECRET_KEY_BASE zwingend (CVE-2026-46386) - war gesetzt. Hocuspocus von `latest` auf Core-Version gepinnt, Memcached 1.6.45. **Vorgehen:** DEV gestaffelt in 6 Etappen, TEST + PROD als Direktsprung in je 2 Commits (Vorbereitung als PreSync-Trockenlauf, dann Upgrade). Downtime je Env 3-7 Min (replicas 1 + Recreate wegen RWO-/tmp-Volume). **Befunde:** `kubernetes/base/apps/openproject/` ist toter Code (keine Application referenziert es) -> geplantes Environment-Pinning unnoetig; die Keycloak-OIDC-Konfiguration ist wirkungslos (Seeder meldet durchgehend `Skipping EnvData::OpenIDConnect::ProviderSeeder`, kein Button, Auth laeuft ueber LDAP). **Lessons:** Hook-only-Aenderungen loesen keinen ArgoCD-Sync aus (Hooks sind aus dem Diff ausgenommen); `kubectl get backup` kollidiert mit dem MariaDB-Operator -> vollqualifizieren; nach dem Sprung `good_jobs` auf fehlgeschlagene Backfill-Jobs pruefen (`Backlogs::MigrateVersionSprintJournalsJob` scheiterte in DEV+PROD im Wettlauf mit dem GoodJob-Scheduler-Start - kein Log und kein Health-Check zeigt das); einmalige `rails runner`-Laeufe nie per exec in den Worker (OOMKilled bei 1Gi) sondern als eigener Job; Worker-Limit PROD auf 2Gi; auf k8s-mgmt-10 zeigte der kubectl-Default-Context auf TEST waehrend die argocd-CLI auf DEV zeigte -> Context immer explizit; `~/git/...` existiert auf MacMini UND mgmt-10 -> absolute Pfade in Anleitungen. Docs: guides/phase-6.3-openproject-update-guide-v2.md (NEU, ersetzt v1). |
