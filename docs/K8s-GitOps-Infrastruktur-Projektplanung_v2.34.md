...
**Erstellt:** 04.02.2026  
**Letzte Aktualisierung:** 15.07.2026 (P2 Drift-Angleich DB-Resilienz-Haertung nach TEST nachgezogen: PriorityClass-App + WAL-Volume 5Gi->8Gi (Online-Resize via spec.walStorage.size, kein Recreate noetig - Korrektur der frueheren Annahme) + max_slot_wal_keep_size 6GB + priorityClassName eneg-stateful-critical an cnpg-erp/cnpg-shared/mariadb-galera; Galera per IST (kein SST); alle Cluster live verifiziert (WAL 8Gi, SHOW=6GB, priority 900000000, Slots aktiv, Alerts feuern nicht). PROD offen (nur cnpg-shared voll + Galera; cnpg-erp braucht nur priorityClass). Docs: migration/dev-to-test-prod-drift-2026-07.md (P2 TEST erledigt), phases/resilienz-haertung-wal-deadlock-dev.md (TEST-Rollout abgehakt))  
...
**DB-Resilienz-Haertung TEST 15.07.2026 abgeschlossen (P2 Drift-Angleich):** 15.07.2026 (PriorityClass-App `priorityclasses` nach TEST (14.07.); WAL-Volume cnpg-erp+cnpg-shared 5Gi->8Gi per Online-Resize (`spec.walStorage.size`, ohne Longhorn-Deadlock, Detach-Trick nicht noetig - widerlegt die fruehere "nur per Recreate"-Annahme aus dem disk-full-Incident 29.06.); `max_slot_wal_keep_size: 6GB` (sighup, live `SHOW`=6GB); `priorityClassName: eneg-stateful-critical` an cnpg-erp/cnpg-shared (via `kubectl cnpg restart`) und mariadb-galera (Operator-Auto-Restart, alle 3 Nodes per IST kein SST, galera-1 ohne Memory-Probleme); alle Pods priority 900000000, beide CNPG 3/3, Galera Ready/GaleraReady; Slots aktiv + CnpgClusterNoPrimary/SlotInactive feuern nicht. Lesson: CNPG-priorityClass ist kein restart-ausloesendes Feld; Primary-Terminating kann lange dauern, loest sich selbst, kein Force-Delete. PROD offen: nur cnpg-shared voll + Galera, cnpg-erp braucht nur priorityClass. Docs: migration/dev-to-test-prod-drift-2026-07.md, phases/resilienz-haertung-wal-deadlock-dev.md)  
**Standort:** Hamburg  
