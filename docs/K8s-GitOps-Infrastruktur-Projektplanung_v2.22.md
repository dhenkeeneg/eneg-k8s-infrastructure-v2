...
## Projektplanung - Version 2.22

**Erstellt:** 04.02.2026  
**Letzte Aktualisierung:** 16.05.2026 (Incident DEV cnpg-erp Frozen Replica + WAL-Stau — Recovery via PVC-Recreation, ca. 90 Min)  
...
| `k8s-{env}-idoit` | i-doit Upload + src (rclone sync) | `upload/`, `src/`, `_backups/` |
...
| 16.05.2026 | 2.22    | **Incident DEV cnpg-erp Frozen Replica + WAL-Stau (~90 Min Recovery).** Ausloeser: Alertmanager-Warnungen `CnpgWalVolumeWarning` auf `cnpg-erp-3-wal` (Primary) und `cnpg-erp-4-wal` (Replica) bei 70-75%, waehrend `cnpg-erp-6-wal` auffaellig bei nur 15% — Cluster meldete trotzdem `healthy 3/3` (CNPG-Readiness-Probe pruefte nur Pod-Health, nicht Replication-Lag). **Wurzelursache:** cnpg-erp-6 seit gestrigem Cluster-Restart (im Zuge der 10.-12.05.-Reparatur) eingefroren auf TL15/LSN `45/4F0000A0`. Streaming-Pfad blockiert (`requested WAL segment 0000000F000000450000004F has already been removed`), Archive-Restore-Pfad blockiert durch verwaiste `00000011.history` im Object Storage (TL17 > Cluster-TL16 → `Refusing to restore future timeline history file`). Die TL17-History stammt aus dem 10.-12.05.-Incident (Promote-Cycle schrieb TL17 in Backup-Catalog, Cluster blieb operativ auf TL16) und lag seitdem latent. Inactive Replication Slot `_cnpg_cnpg_erp_6` hielt ~1,2 GiB WALs auf Primary fest → 75%-Volume-Belegung. **Recovery via PVC-Recreation (Option A — saubere Loesung ueber `pg_basebackup` statt Archive-Restore, umgeht damit auch das parallele Backup-Subsystem-Problem):** (1) Stale Slot manuell gedroppt (`pg_drop_replication_slot('_cnpg_cnpg_erp_6')`), (2) Pod `cnpg-erp-6` geloescht, (3) PVCs `cnpg-erp-6` + `cnpg-erp-6-wal` geloescht → CNPG-instance-serial-Bump 6→7. **Zwischenereignis:** Waehrend des Reconciles kam es zu einem ungeplanten Failover cnpg-erp-3 → cnpg-erp-4 (Liveness-Probe-Stress + zusaetzliche `pg_basebackup`-Last), neue valide Timeline 17 entstand. cnpg-erp-3 nahm danach als Replica wieder am Cluster teil. (4) Zombie-Pod cnpg-erp-6 (Pod blieb 2/2 Running, aber CNPG hatte ihn aus `instanceNames` entfernt) zum Schluss aufgeraeumt. **Endzustand:** Cluster `Cluster in healthy state`, READY 3/3 (Primary cnpg-erp-4, Replicas cnpg-erp-3 + cnpg-erp-7), beide Slots `active=t, lag_size=0 bytes`, WAL-Belegung 15% / 15% / 2%, `CnpgWalVolumeWarning`-Alerts aufgeloest. cnpg-shared war nicht betroffen. **Begleitbefund (Problem B, separat zu behandeln):** Backup-Subsystem zeigt seit gestern `failed` mit `exit status 4` fuer alle Backup-CRs beider Cluster — `barman-cloud-backup` selbst laeuft erfolgreich durch, aber nachgelagertes `barman-cloud-backup-show` und `backup-delete` laufen in 5-Min-Read-Timeouts gegen NAS10 QuObjects (LIST auf `cnpg-erp/cnpg-erp/base/`). WAL-Archiving funktioniert unbeeintraechtigt. Ursache temporaere QNAP-Last durch grosse parallele NAS-Backupjobs; eskalierende Konsequenzen ohne Eingriff: Retention raeumt nicht auf → Bucket waechst → LIST noch langsamer. Plan Phase 2: Bucket-Cleanup via `s3cmd`, Timeout-Erhoehung als Notbremse, strategisch Garage S3 als Migrations-Option in separater Session. **Dokumentation:** Neue Dateien `docs/incidents/2026-05-16-cnpg-erp-frozen-replica.md` (vollstaendiger Incident-Bericht mit 5 Lessons Learned) und `docs/runbooks/cnpg-frozen-replica-stale-slot.md` (generisches Runbook mit Diagnose-Queries, Reparatur-Schritten, Object-Storage-Cleanup, PrometheusRule-Vorschlag fuer Slot-Lag-Frueherkennung). **Offen:** TL17-History im Object Storage pruefen/aufraeumen; PrometheusRule fuer Slot-Inactivity erwaegen; Phase 2 Backup-Subsystem-Reparatur. |
