...
## Projektplanung - Version 2.36

**Erstellt:** 04.02.2026  
**Letzte Aktualisierung:** 16.07.2026 (P4a Drift-Angleich Thanos S3-Migration NAS10->NAS20 TEST erledigt (PROD offen): Bloecke NAS10->NAS20 kopiert (volle Historie, Compactor-Pause via compactor.enabled:false - replicaCount greift beim bitnami-Chart NICHT), dann Secret thanos-objstore-config (endpoint nas20, insecure:false, ca_file:/etc/ca/ca.crt) + CA-Mounts an compactor/storegateway (monitoring-thanos) und Prometheus-Thanos-Sidecar (monitoring: prometheusSpec.volumes+thanos.volumeMounts) split-brain-sicher in einem Cutover. CA-Secret eneg-s3-ca war durch Loki-Migration bereits vorhanden -> wiederverwendet (kein neues Secret). Secret-first-Sync-Reihenfolge. Live verifiziert: Storegateway laedt 15 Bloecke von NAS20 (kein x509), Compactor Cleanup sauber, Sidecar ready mit CA (Upload beim naechsten 2h-Cut - Admin-API aus, kein erzwungener Block-Cut), Query nutzt Storegateway. Stolpersteine: (1) compactor.replicaCount wirkungslos -> enabled:false; (2) falscher secret_key -> InvalidAccessKeyId trotz TLS-OK (Access-Key-Format s3-k8s-{env}:<key>, mit funktionierendem loki-s3-credentials abgleichen); (3) ArgoCD "Synced" ohne echten Apply -> expliziter Sync noetig, CLI immer mit --context. Nebenbefund (NICHT P4a, dokumentiert): Thanos-Query sidecarsService kube-prometheus-stack-thanos-discovery existiert in KEINEM Cluster (real: prometheus-operated:10901) -> Query bezieht Historie via Storegateway, findet Sidecar-Store nicht direkt; vorbestehend/cluster-uebergreifend, eigener base-Fix spaeter. P4b (CNPG-backup, MariaDB-physical, Garage, Odoo, i-doit NAS10->NAS20) weiterhin offen. Docs: phases/phase-14-p4a-thanos-nas20-test-prod.md, migration/dev-to-test-prod-drift-2026-07.md (P4a TEST erledigt))  
